...
- No build tools — just static files, so it works with GitHub Pages out of
  the box.
- Fonts (Space Grotesk, Inter, IBM Plex Mono) load from Google Fonts via a
  CDN link in `index.html`.
- The hero uses a real 3D scene (WebGL, via the Three.js library loaded
  from a CDN in `index.html`) — a small rotating neural-network graph with
  glowing nodes and traveling signal pulses. It tilts toward your cursor
  on desktop. If the CDN is ever blocked or WebGL isn't available, it falls
  back to a soft gradient background automatically, no broken page.
- Project cards tilt in 3D toward the cursor on hover, with a moving glare
  highlight. Skipped automatically on touch devices.
- Small floating 3D shapes drift through the About, Skills, and Contact
  sections for visual consistency with the hero.
- Everything respects `prefers-reduced-motion` and the site is responsive
  down to mobile.
